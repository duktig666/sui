...
# Test ENV
参看 dex-sui/docker/dex-dev 可以再起一套docker compose环境用来测试，注意端口映射问题 配置到dex-sui/docker/dex-test

# phase6 - 订单薄索引优化
现在的订单薄的实现是 CheckPoint机制，在链上发出全量订单薄的事件。
虽然还有进行真实的压测，但是可以遇到到一些性能问题：
1. 受限于sui的CheckPoint机制，在高频交易的场景，不能满足dex对延时的要求。
2. 全量订单薄数据流比较大，可以考虑使用增量的事件推送，链下索引和api构建订单薄
关于订单薄可以参看dydx的设计，当然也需要结合sui的真实情况，可以考虑在交易共识之后，CheckPoint之前，发出stream，通过链下订阅的方式，快速将需要高延迟的事件发出。
需要先设计整体框架结合dydx和sui，然后发出对应的stream事件，链下索引，构建订单簿，然后api和ws对外暴露。
框架设计需要通用化，后续还可能其他事件需要这样的方式。
这里我还有可能有很多没有考虑到的地方，也帮我完善，有疑问的地方让我做出决策。
输出规划和对应的架构和技术方案 输出到sui/mynotes/dex/analyst/phase6 文件夹下，先不实现代码，等我review方案再说。

---
phase5和phase6 已经规划好了，phase5需要等待另外两个工程师实现，可以先等待搁置。
如果先实现phase6是否有冲突，评估可行性。

问题：
1. 快速通道事件触发时机为 AuthorityState 执行后回调，是否有问题？还是否有其他选择，进行对比
2. api和orderbook推送订单薄还是按照全量推送，事件发出和索引按照增量。客户端增量渲染比较复杂。
3. dydx的订单薄是否有双通道验证，订单薄数据是否需要双通道验证，如果双通道验证 CheckPoint发出全量还是增量事件，如果是全量事件岂不是带宽还是有压力。是否可以不发出全量订单薄事件？dydx会发出最终状态的全量快照吗？
4. 这个阶段的规划中，除了订单簿要走快速通道外，是否还有其他功能也规划在快速通道？
5. 快速通道是否需要定义新的事件类型，用以和CheckPoint通道的事件进行区分? 另外如果使用新的事件类型是不是就不需要过滤了，即是像dydx那样引入类似OffChainUpdates的新类型，还是复用sui的事件？需要深入分析和抉择。
6. DexStreamingManager 事件的发出需要全节点还是验证者节点？ 我看到规划中通过NodeConfig来配置（全节点是否可以发出，还是必须要验证者）
7. dex-streamer 是要新增加服务吗？单进程使用`tokio::broadcast` channel？直接使用gRPC streaming区别，可以考虑直接使用gRPC streaming一步到位
8. L2 Book和L3 Book区别？
根据上述问题调整sui/mynotes/dex/analyst/phase6下的规划方案，有决策问题及时让我做出决策。另外将上述问题在sui/mynotes/dex/analyst/phase6下用一个新文档回复一下

phase6的规划是否涉及dex-test-panel、dex-indexer-e2e-test、dex-node-test 的适配完善。 以及对于ws和api有哪些调整？

将phase6的规划文档总结成架构和技术方案文档输出到 dex-sui/docs/indexer/tech/orderbook-grpc-stram-tech.md，不需要引用，写完整文档。
...
│          项目          │ 优先级 │                        说明                         │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ GetSnapshot 实现       │ P0     │ gRPC GetSnapshot 方法返回                           │
│                        │        │ unimplemented，无法做快照恢复                       │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ 定期对账 (Reconciler)  │ P0     │ reconciler 模块未实现，当前仅靠序列号间隙检测       │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ 监控指标               │ P1     │ 11 个核心 Prometheus 指标均未实现，仅靠日志观测     │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ BBO 独立模块           │ P1     │ BBO 直接内联在 L2Book 方法中，无独立 tracker        │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ WS 增量频道            │ P1     │ 目前 WS 只推全量快照，无增量频道                    │
│ (l2BookDelta)          │        │                                                     │
├────────────────────────┼────────┼─────────────────────────────────────────────────────┤
│ 推送频率控制           │ P1     │ 无 rate limiting（Hyperliquid 是 0.5s）             │
└────────────────────────┴────────┴─────────────────────────────────────────────────────┘

---
刚才合并了另一个工程师关于杠杆倍数、高级订单、标记价格、资金费率、清算的订单和成交的功能，合
并的远程分支feature-match-engine。 新功能的技术文档： dex-sui/docs/funding-rate-design.md dex-sui/docs/leverage-and-isolated-margin.md dex-sui/docs/liquidation-design.md
先进行下全面测试 dex-dev看是否要重新运行

# phase5 - 功能进一步完善
现在规划phase5。有另外两个工程师，其中一个在实现真正的链上的跨链充值和提现（现在的充值是模拟的不是最终方案）；
另一个工程师在实现如下功能：
1. 合约的逐仓相关功能
2. 限价止盈止损订单，市价止盈止损订单
3. 预言机（标记价格）
...
5. 清算
6. 杠杆倍率的调整
这两个工程师实现的功能需要indexer、api、ws等进行适配，必要部分发出指定事件，标准是符合Hyperliquid的api和ws规范。
可能影响的部分有：
1. 新的订单类型
2. 标记价格
3. 资金费率（现在是如何实现的？是否是有问题的）
4. 清算的订单和成交
5. 充值和体现记录
6. 杠杆倍率的设置
7. 全仓和逐仓
现在我正在等待这两个工程师实现完这些功能后，然后开始实施phase5，在此之前先进行规划，是否有现阶段可以做的。 规划文档输出到 sui/mynotes/dex/analyst/phase5 下

---
phase5其中一位工程师已经实现了 杠杆倍数、高级订单openhands 记价格、资金费率、清算的订单和成交的功能，代码已经合并。另一位工程师的充提功能还在实现中。
新功能的技术文档： dex-sui/docs/funding-rate-design.md dex-sui/docs/leverage-and-isolated-margin.md dex-sui/docs/liquidation-design.md
我们是否可以先兼容实现这位工程师的功能。
