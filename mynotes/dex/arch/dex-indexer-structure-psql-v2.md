# Sui DEX Indexer 架构结构图 (PostgreSQL 版本)

> 基于 `dex-indexer-analyst.md` 文档生成的完整架构图
> **存储层**: Redis + PostgreSQL (两层架构)
...
flowchart TB
    ...
    subgraph Storage["存储层 (两层架构)"]
        ...
        subgraph L2["L2 持久层 - PostgreSQL (&lt;10ms)"]
            PostgreSQL["PostgreSQL"]
            P1["K线数据 (candles)"]
            P2["成交记录 (fills)"]
            P3["资金费率 (funding_rates)"]
            P4["订单记录 (orders)"]
            P5["仓位历史 (positions)"]
            P6["历史订单归档"]
            P7["仓位快照归档"]
    ...
    %% 存储写入
    OP -->|"订单簿/订单状态"| Redis
    TP -->|"K线/成交记录"| PostgreSQL
    TP -->|"已确认成交量"| Redis
    HP -->|"历史订单/仓位快照"| PostgreSQL

    %% API 读取
    REST -->|"查询"| Redis
    REST -->|"历史K线/订单"| PostgreSQL
    WS -->|"订阅"| Redis
    ...
    class Redis,PostgreSQL,R1,R2,R3,R4,P1,P2,P3,P4,P5,P6,P7,P8 storage
    class REST,WS api
...
### 批量通道 (Checkpoint)
```
Checkpoint Store → sui-indexer-alt → Kafka → Trade/History Processor → PostgreSQL
延迟: 3-5s
用途: K线聚合、成交持久化、历史数据
```
...
| K线数据 (candles) | Checkpoint | PostgreSQL | 历史K线查询 |
| 成交记录 (fills) | Checkpoint | PostgreSQL | 成交历史 |
| 资金费率 (funding_rates) | Checkpoint | PostgreSQL | 资金费率历史 |
| 订单记录 (orders) | Checkpoint | PostgreSQL | 订单历史 |
| 仓位历史 (positions) | Checkpoint | PostgreSQL | 仓位变更记录 |
| 历史订单归档 | Checkpoint | PostgreSQL | 长期存储/分析 |
| 仓位快照归档 | Checkpoint | PostgreSQL | 长期存储/分析 |
| 交易分析视图 | Checkpoint | PostgreSQL | OLAP 分析 (物化视图) |
...
相比 TimescaleDB + ClickHouse 方案，PostgreSQL 版本的特点：

| 维度 | PostgreSQL 版本 | TimescaleDB + ClickHouse 版本 |
|------|----------------|------------------------------|
| **部署复杂度** | 低 (单一数据库) | 高 (多数据库协调) |
| **运维成本** | 低 | 高 |
| **时序查询性能** | 中等 | 高 (TimescaleDB 优化) |
| **OLAP 分析性能** | 中等 | 高 (ClickHouse 优化) |
| **适用场景** | 中小规模、快速迭代 | 大规模、高性能要求 |
| **数据一致性** | 简单 (单库事务) | 复杂 (跨库同步) |

### PostgreSQL 优化建议

1. **分区表**: 对 `fills`、`candles` 等时序数据按时间分区
2. **物化视图**: 用于交易分析等 OLAP 查询
3. **索引优化**: 针对常用查询建立复合索引
4. **连接池**: 使用 PgBouncer 管理连接
5. **读写分离**: 可配置读副本分担查询压力
